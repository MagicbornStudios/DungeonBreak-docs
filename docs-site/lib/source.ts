...
export const getSource = cache(async () => {
	...
	let fileSource: Source<{ metaData: MetaData; pageData: any }> = {
		files: [],
	};
	try {
		// .source/server is emitted by fumadocs-mdx (run after docs:generate)
		const mod = await import("../.source/server");
		const docsResult = await mod.docs;
		if (docsResult?.toFumadocsSource) {
			fileSource = docsResult.toFumadocsSource();
		}
	} catch {
		// No file-based docs (e.g. .source not generated yet)
	}
	const mergedFiles = [
		...payloadSource.files,
		...(fileSource.files as VirtualFile[]),
	];
	return loader({
		baseUrl: "/docs",
		source: { files: mergedFiles },
	});
});
