...
const PLANNING_REPORTS = path.resolve(process.cwd(), "..", ".planning", "reports");
...
export async function GET(request: Request) {
  try {
    ...
    const metricsPath = path.join(PLANNING_REPORTS, "metrics.jsonl");
    const usagePath = path.join(PLANNING_REPORTS, "usage.jsonl");
