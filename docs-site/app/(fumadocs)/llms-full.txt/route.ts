...
function extractTextFromLexical(content: any): string {
  ...
  function extractFromNode(node: any): string {
    ...
    if (node.type === "heading") {
      ...
      const parsedDepth = Number.parseInt(String(tag).substring(1), 10);
      const depth = Number.isFinite(parsedDepth)
        ? Math.min(6, Math.max(1, parsedDepth))
        : 2;
...
export async function GET() {
  ...
  const llmTexts = pages.map((page) => {
    ...
    const content = extractTextFromLexical(data.content);

    return `%23 ${data.title ?? page.data.title}
URL: ${page.url}
