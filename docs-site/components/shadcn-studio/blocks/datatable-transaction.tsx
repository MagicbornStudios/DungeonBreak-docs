...
export const columns: ColumnDef<Item>[] = [
    ...
    cell: ({ row }) => (
      <div className='flex items-center gap-2'>
        ...
        <div className='flex flex-col text-sm'>
          <span className='text-card-foreground font-medium'>{row.getValue('name') as string}</span>
  ...
  },
  {
    ...
    header: 'Status',
    cell: ({ row }) => (
      <Badge className='bg-primary/10 text-primary rounded-sm px-1.5 capitalize'>{String(row.getValue('status'))}</Badge>
    )
  },
  {
    accessorKey: 'paidBy',
