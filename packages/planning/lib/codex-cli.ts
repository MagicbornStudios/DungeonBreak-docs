...
import { logCodexAuth } from "@planning/lib/codex-auth-log";
...
/** Run Codex with the given prompt; returns the assistant text. workingDirectory defaults to process.cwd(). */
export async function runCodexExecJson(
  prompt: string,
  workingDirectory?: string | CodexExecOptions,
): Promise<string> {
  const opts: CodexExecOptions =
  ...
  const threadOptions: { sandboxMode: string; skipGitRepoCheck: boolean; workingDirectory: string; model?: string } = {
    sandboxMode: "read-only",
    skipGitRepoCheck: true,
    workingDirectory: cwd,
  };
  if (opts.model?.trim()) threadOptions.model = opts.model.trim();
...
/** Stream Codex response; yields text chunks. Uses runStreamed when available, else yields full response once. */
export async function* runCodexExecJsonStream(
  prompt: string,
  ...
  if (opts.model?.trim()) threadOptions.model = opts.model.trim();
  const thread = codex.startThread(threadOptions);
  const runStreamed = (thread as { runStreamed?: (p: string) => Promise<{ events: AsyncIterable<unknown> }> }).runStreamed;
  if (typeof runStreamed === "function") {
    const { events } = await runStreamed.call(thread, prompt);
    for await (const event of events) {
      const e = event as { type?: string; item?: { type?: string; content?: string; text?: string } };
      if (e.type === "item.completed" && e.item?.type === "message" && typeof e.item.text === "string") {
        yield e.item.text;
      }
    }
    return;
  ...
  const turn = await thread.run(prompt);
  const full = String(turn.finalResponse ?? "").trim();
  if (full) yield full;
}
