...
import { Tabs, TabsList, TabsTrigger } from "@/planning-ui/ui/tabs";
import { ScrollArea } from "@/planning-ui/ui/scroll-area";
...
const DIFF_REMOVE_CLASS = "bg-red-500/10 text-red-700 dark:text-red-400";
const DIFF_ADD_CLASS = "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400";
          ...
          <div className="flex shrink-0 flex-col border-r border-border/50 bg-muted/30">
            {oldLines.map((_, i) => (
              <div
                key={`old-${i}`}
                className={cn(
                  "flex h-5 w-10 shrink-0 items-center justify-end pr-2",
                  changeMap.get(i + 10000) === "remove" ? DIFF_REMOVE_CLASS : "text-muted-foreground",
                )}
              >
                {i + 1}
              </div>
          ...
          <div className="flex shrink-0 flex-col border-r border-border/50 bg-muted/30">
            {newLines.map((_, i) => (
              <div
                ...
                className={cn(
                  "flex h-5 w-10 shrink-0 items-center justify-end pr-2",
                  changeMap.get(i) === "add" ? DIFF_ADD_CLASS : "text-muted-foreground",
                )}
              >
                {i + 1}
              </div>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col rounded-lg border border-border/60 bg-muted/10">
      <div className="flex flex-none items-center justify-between gap-2 border-b border-border/60 px-3 py-2">
        <Tabs value={viewMode} onValueChange={(v: string) => setViewMode(v as "summary" | "code")}>
              ...
              {edits.map((e, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={cn("ml-auto shrink-0 text-[10px]", DIFF_ADD_CLASS)}>+{stats.add}</span>
                    <span className={cn("shrink-0 text-[10px]", DIFF_REMOVE_CLASS)}>-{stats.remove}</span>
                  </Button>
                );
              })}
            </ScrollArea>
